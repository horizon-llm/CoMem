...
from agent_system.multi_turn_rollout.swe_function_match_v2 import action_consistency_reward, parse_action, Action, action_consistency_reward_detailed
...
class SumAPIRespRecentLiteLLMGLMTrajectoryCollector(TrajectoryCollector):
    def __init__(self, config, tokenizer: PreTrainedTokenizer, processor=None):
        """
        Initialize the SumAPIRespRecentLiteLLMGLMTrajectoryCollector class using litellm for API calls.
        ...
        async def process_all():
            ...
            async def process_item(idx: int):
                ...
                if retain_turns > 0:
                    ...
                    #     f"{history_to_append}\n"
                    # )
                    format_prompt = load_prompt_template(self.config.env.response_agent.get("query_prompt_template_path", ""))
    ...
    def vanilla_multi_turn_loop(
        ...
        reward_details_list = []
        for i in range(batch_size):
            _, text_action_parsed = parse_action(text_actions[i], use_tool_call=self.config.env.swebench.get('use_tool_call', False), glm_model=True)
            ...
            # reward = action_consistency_reward(text_action_parsed, expected_action_parsed)
