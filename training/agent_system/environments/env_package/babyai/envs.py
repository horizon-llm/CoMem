...
from agent_system.environments.env_package.babyai.agentenv_babyai import BabyAIEnvClient
...
class BabyAIWorker:
    ...
    def __init__(self, env_server_base: str):
        self.env = BabyAIEnvClient(env_server_base, data_len=1, timeout=2400)
    ...
    def step(self, action):
        ...
        info = copy.deepcopy(self.env.info)
        info['won'] = copy.deepcopy(done and reward > 0.0)
        reward = reward if done else 0.0 # a reward is only revealed at the end of an episode
        info['task_score'] = copy.deepcopy(reward)
...
class BabyAIMultiProcessEnv(gym.Env):
    ...
    def __init__(
        ...
        if not is_train:
            self.item_ids = json.load(open("agent_system/environments/env_package/babyai/agentenv_babyai/data/eval/babyai_test.json", 'r'))
        else:
            self.item_ids = json.load(open("agent_system/environments/env_package/babyai/agentenv_babyai/data/train/babyai_train.json", 'r'))

        self.group_n = group_n
        env_num = min(len(self.item_ids), env_num)
        ...
        for i in range(self.num_processes):
            worker = BabyAIWorker.remote(env_server_base)
