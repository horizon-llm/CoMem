...
from agent_system.environments.env_package.textcraft.agentenv_textcraft import TextCraftEnvClient

@ray.remote(num_cpus=0.2)
class TextCraftWorker:
    """
    ...
    def __init__(self, env_server_base: str):
        self.env = TextCraftEnvClient(env_server_base, data_len=1, timeout=2400)
...
class TextCraftMultiProcessEnv(gym.Env):
    ...
    def __init__(
        ...
        if not is_train:
            self.item_ids = json.load(open("agent_system/environments/env_package/textcraft/agentenv_textcraft/data/eval/textcraft_test.json", 'r'))
        else:
            self.item_ids = json.load(open("agent_system/environments/env_package/textcraft/agentenv_textcraft/data/train/textcraft_train.json", 'r'))
        ...
        for i in range(self.num_processes):
            worker = TextCraftWorker.remote(env_server_base)
