...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    metrics = {
        ...
        **({f"episode/{k}": v[0].item() for k, v in batch.non_tensor_batch.items() if "success_rate" in k}),
        # action rewards
        "action_rewards/mean": 
            batch.non_tensor_batch["action_rewards_global_mean"][0] if "action_rewards_global_mean" in batch.non_tensor_batch else 0,
        "length_penalty/mean":
            batch.non_tensor_batch["length_penalties_mean"][0] if "length_penalties_mean" in batch.non_tensor_batch else 0,
    }
    return metrics
