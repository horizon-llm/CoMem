...
class RayPPOTrainer:
    ...
    def _maybe_log_val_generations(self, inputs, outputs, scores, formatted_prompts=None, text_actions=None, expected_actions=None, reward_details=None):
        ...
        if formatted_prompts is not None:
            samples = list(zip(inputs, outputs, scores, formatted_prompts, text_actions, expected_actions, reward_details))
        else:
    ...
    def _validate(self):
        ...
        sample_outputs = []
        sample_scores = []
        sample_formatted_prompts = []
        sample_text_actions = []
        sample_expected_actions = []
        sample_reward_details = []

        # for test_data in self.val_dataloader:
        # Add progress bar for validation
        val_progress = tqdm(enumerate(self.val_dataloader), desc="Validation", leave=False, total=len(self.val_dataloader))
        for batch_idx, test_data in val_progress:
            ...
            if "formatted_prompts" in test_batch.non_tensor_batch:
                sample_formatted_prompts.extend(test_batch.non_tensor_batch["formatted_prompts"])
            if "text_actions" in test_batch.non_tensor_batch:
                sample_text_actions.extend(test_batch.non_tensor_batch["text_actions"])
            if "expected_actions" in test_batch.non_tensor_batch:
                sample_expected_actions.extend(test_batch.non_tensor_batch["expected_actions"])
            if "reward_details" in test_batch.non_tensor_batch:
                sample_reward_details.extend(test_batch.non_tensor_batch["reward_details"])
        ...
        if len(sample_formatted_prompts) > 0:
            self._maybe_log_val_generations(
                inputs=sample_inputs,
                outputs=sample_outputs,
                scores=sample_scores,
                formatted_prompts=sample_formatted_prompts,
                text_actions=sample_text_actions,
                expected_actions=sample_expected_actions,
                reward_details=sample_reward_details,
            )
        else:
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if "tools_kwargs" in batch.non_tensor_batch:
                    non_tensor_batch_keys_to_pop.append("tools_kwargs")
                if "response" in batch.non_tensor_batch:
                    non_tensor_batch_keys_to_pop.append("response")
                if "turns" in batch.non_tensor_batch:
                    non_tensor_batch_keys_to_pop.append("turns")
                gen_batch = batch.pop(
                    batch_keys=batch_keys_to_pop,
                    non_tensor_batch_keys=non_tensor_batch_keys_to_pop,
                ...
                with _timer("step", timing_raw):
                    ...
                    if rollout_data_dir:
                        with _timer("dump_rollout_generations", timing_raw):
                            ...
                            if 'conv_strings' in batch.non_tensor_batch:
                                conv_strings = batch.non_tensor_batch['conv_strings']
                                reward_extra_infos_dict['conv_strings'] = conv_strings.tolist()
