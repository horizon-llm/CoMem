...
class ValidationGenerationsLogger:
    ...
    def log_generations_to_wandb(self, samples, step):
        ...
        if len(samples[0]) > 3:
            columns = ["step"] + sum([[f"input_{i + 1}", f"output_{i + 1}", f"score_{i + 1}", f"query_prompt_{i + 1}", f"text_action_{i + 1}", f"expected_action_{i + 1}", f"reward_details_{i + 1}"] for i in range(len(samples))], [])
        else:
