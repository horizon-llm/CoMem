...
for bs in ${batch_sizes[@]}; do
  for inp_len in ${input_lengths[@]}; do
    for out_len in ${output_lengths[@]}; do
      ...
      OUTFILE="vllm_bench_cpu_batch${bs}_inp${inp_len}_outp${out_len}_repeat${r}.out"
      if [ -f "$OUTFILE" ]; then
      ...
      batch_size=$bs input_length=$inp_len output_length=$out_len repeat=$r bash scripts/vllm_bench_batch_deepswe_cpu.sh
