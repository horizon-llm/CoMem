...
EXP_NAME="comem-grpo-qwen3-4b-grp16-l2048-r1-deepswe-sweverified-eval-t100-kso-r2-lat"
...
PYTHON="/root/miniconda3/envs/r2e-gym/bin/python" DEVICES="0,1,2,3" nohup bash scripts/start_vllm_server.sh > "vllm_server.out" 2>&1 &
VLLM_PID=$!
PYTHON="/root/miniconda3/envs/r2e-gym/bin/python" DEVICES="4,5" MODEL=$MEM_MODEL MAX_CONTEXT_LEN=65536 nohup bash scripts/start_vllm_server_with_sync.sh > "vllm_mem_model_server.out" 2>&1 &
VLLM_MEM_PID=$!

# Set up trap to kill the server processes on script exit (normal or error)
trap "pkill -P $VLLM_PID $VLLM_MEM_PID 2>/dev/null; kill $VLLM_PID $VLLM_MEM_PID 2>/dev/null || true" EXIT
