...
EXP_NAME="comem-grpo-qwen3-4b-grp16-l2048-r1-deepswe-sweverified-eval-t100-kso-r2-lat-${GPU_NAME}"
...
wait_for_servers() {
    local urls=("$@")
    local max_wait=${MAX_WAIT:-2000}  # Default ~33 minutes
...
python src/r2egym/agenthub/run/edit_summ_kso.py runagent_multiple \
  ...
  --backend "kubernetes" \
  --scaffold "r2egym" \
  --exp_name "$EXP_NAME-tp4bcpu-w${MAX_WORKERS}-d${DATA_SEED}-n${NUM_SAMPLES}" \
  --temperature 1.0 \
  --max_steps 40 \
  --use_existing False \
  --skip_existing False \
