...
EXP_NAME="full-context-glm-8gpu-eval-t100-lat-cpu"
...
PYTHON="/root/miniconda3/envs/r2e-gym/bin/python" DEVICES="0,1,2,3,4,5,6,7" KV_OFFLOADING_SIZE=500 nohup bash scripts/start_vllm_server_glm_cpu.sh > "vllm_server.out" 2>&1 &
VLLM_PID=$!
...
python src/r2egym/agenthub/run/edit.py runagent_multiple \
  ...
  --backend "kubernetes" \
  --scaffold "r2egym" \
  --exp_name "$EXP_NAME-cpu-w${MAX_WORKERS}-d${DATA_SEED}-n${NUM_SAMPLES}" \
  --temperature 1.0 \
  --max_steps 40 \
  --use_existing False \
  --skip_existing False \
