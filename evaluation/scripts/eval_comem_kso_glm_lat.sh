...
EXP_NAME="comem-grpo-grp16-max40-rewardv2-s35-l2048-r1-glm-newquerypromptv5-sweverified-eval-glm-t100-kso-r4-lat"
...
PYTHON="/root/miniconda3/envs/r2e-gym/bin/python" DEVICES="4,5,6,7" MODEL=$MEM_MODEL MAX_CONTEXT_LEN=65536 nohup bash scripts/start_vllm_server_with_sync.sh > "vllm_mem_model_server.out" 2>&1 &
VLLM_MEM_PID=$!
...
python src/r2egym/agenthub/run/edit_summ_kso.py runagent_multiple \
  ...
  --backend "kubernetes" \
  --scaffold "r2egym" \
  --exp_name "$EXP_NAME-tp4-w${MAX_WORKERS}-d${DATA_SEED}-n${NUM_SAMPLES}" \
  --temperature 1.0 \
  --max_steps 40 \
  --use_existing False \
  --skip_existing False \
