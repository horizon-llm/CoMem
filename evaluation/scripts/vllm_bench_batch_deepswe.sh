...
PYTHON="/root/miniconda3/envs/r2e-gym/bin/python" DEVICES=0,1 nohup bash scripts/start_vllm_server.sh > "vllm_server.out" 2>&1 &
VLLM_PID=$!
...
PYTHON="$CONDA_PREFIX/bin/python" vllm bench serve \
  ...
  --random-input-len $input_length | tee vllm_bench_batch${batch_size}_inp${input_length}_outp${output_length}_repeat${repeat}.out
