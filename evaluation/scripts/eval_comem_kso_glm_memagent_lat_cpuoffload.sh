...
EXP_NAME=${EXP_NAME:-"comem-kso-glm-memagent-iterative-t100-r${RETAIN_MOST_TURNS}-lat-cpu"}
...
# 1) Start RL-MemoryAgent vLLM backend (with CPU KV offloading)
...
nohup bash scripts/start_vllm_server_with_sync_cpu.sh > "${EXP_NAME}_vllm_memagent_backend.out" 2>&1 &
VLLM_MEM_PID=$!
...
python src/r2egym/agenthub/run/edit_summ_kso.py runagent_multiple \
  ...
  --backend "kubernetes" \
  --scaffold "r2egym" \
  --exp_name "$EXP_NAME-w${MAX_WORKERS}-d${DATA_SEED}-n${NUM_SAMPLES}" \
  --temperature 1.0 \
  --max_steps ${MAX_STEPS} \
  --use_existing False \
  --skip_existing False \
