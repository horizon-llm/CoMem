#!/bin/bash
# Latency benchmark for SYNCHRONOUS COMEM-KSO agent
#
# Identical config to run_latency_benchmark.sh but uses the sync agent
# (memory compression runs to completion before agent model query).
#
# Run this AFTER restarting vLLM servers to avoid prefix cache reuse
# from the async benchmark.
#
# Usage:
#   bash scripts/run_latency_benchmark_sync.sh
#
# Environment variables (all optional, with defaults):
#   MODEL              - Agent model name
#   MEM_MODEL          - Memory/summarization model
#   NUM_INSTANCES      - Number of instances per config (default: 16)
#   INPUT_TARGETS      - Comma-separated input token targets
#   OUTPUT_TARGETS     - Comma-separated output token targets
#   MEM_OUTPUT_TARGETS - Comma-separated memory output token targets
#   STEPS_LIST         - Comma-separated step counts
#   WORKERS_LIST       - Comma-separated worker/concurrency counts
#   RETAIN_MOST_TURNS  - Compression frequency (default: 3)
...
EXP_NAME="sync-latency-bench-${GPU_NAME}-$(date +%Y%m%d_%H%M%S)"
...
RETAIN_MOST_TURNS=${RETAIN_MOST_TURNS:-3}

echo ""
echo "=============================================="
echo "COMEM-KSO SYNC Latency Benchmark"
echo "=============================================="
echo "Model:              $MODEL"
...
echo "Retain most turns:  $RETAIN_MOST_TURNS"
echo "=============================================="
echo ""
