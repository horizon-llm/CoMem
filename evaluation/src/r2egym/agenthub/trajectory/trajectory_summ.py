...
# ##############################################################################
class TrajectoryStep_Summ(BaseModel):
    step_idx: int
    ...
    token_usage_summary: int
    token_usage_summary_prompt: int
    ...
    env_exec_time: float
    summ_exec_time: float
    total_step_time: float
    total_time_traj: float
    step_count: int
    summary_finish_reason: Optional[str] = None  # reason for summary exit

    # kso related stats
    prefill_exec_time: Optional[float] = None
    summary_start_time: Optional[float] = None
    summary_finish_time: Optional[float] = None
    agent_start_time: Optional[float] = None
    agent_finish_time: Optional[float] = None
...
# ##############################################################################
class Trajectory_Summ(BaseModel):
    ...
    ##############################
    trajectory_steps: list[TrajectoryStep_Summ]
    ...
    system_prompt: Optional[str] = None  # prompt shown to LLM
    user_prompt: Optional[str] = None  # initial user/instance prompt
    ...
    traj_start_time: Optional[float] = None  # trajectory start time
    traj_finish_time: Optional[float] = None  # trajectory finish time
    ...
    # kso related stats
    ##############################
    retain_most_turns: Optional[int] = None
    use_optimized_schedule: Optional[bool] = None

    @classmethod
    def load_from_model_dump_json(cls, json_string: str):
        return Trajectory_Summ.model_validate_json(json_string)
