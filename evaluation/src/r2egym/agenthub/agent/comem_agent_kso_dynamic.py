"""
Implement the k-step-off version (Comem-KSO), with dynamic adjustment on k
"""
import os
import re
import copy
...
class AgentArgs:
    ...
    retain_most_turns: int = 1 # number of most recent turns to retain without compression before max steps
    retain_most_turns_after_limit: int = 1 # number of most recent turns to retain without compression after exceeding max steps
...
class COMEMAgent:
    ...
    def run(
        ...
        steps_since_last_compression = 0  # Counter for steps since last compression triggered
        ...
        while not done:
            ...
            # Determine effective retain_most_turns based on whether we've hit max_steps
            steps_remaining = max_steps - step_count
            if steps_remaining <= 0:
                effective_retain_most_turns = self.args.retain_most_turns_after_limit
            else:
                effective_retain_most_turns = self.args.retain_most_turns

            should_compress_this_step = steps_since_last_compression >= (effective_retain_most_turns - 1) and len(self.trajectory_steps) > 0
            ...
            if should_compress_this_step:
                ...
                steps_since_last_compression = 0  # Reset counter after triggering compression
                if self.args.use_optimized_schedule:
            ...
            if not should_compress_this_step:
                steps_since_last_compression += 1
