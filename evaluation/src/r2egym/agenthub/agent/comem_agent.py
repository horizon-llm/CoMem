...
MAX_CONTEXT_TOKENS = 131072  # Example max context length, adjust as needed
...
class COMEMAgent:
    ...
    def _setup_endpoints(self):
        """
        Build a client pool based on model name.
        If model_name is in OPENAI_MODELS, use OpenAI API with API key from environment.
        Otherwise, use vLLM endpoints with addresses from config.
        """
        model_name = self.args.memory_model_name
        timeout_s = getattr(self.args, "timeout_s", 60)

        # Check if model is an OpenAI model
        if model_name in OPENAI_MODELS:
            # Use OpenAI API with API key from environment
            self.client = AsyncOpenAI(api_key=os.getenv("OPENAI_API_KEY"))
            self._use_openai = True
            logger.info(f"Using OpenAI API with model: {model_name}")
        else:
            # Use vLLM endpoints with custom base URLs
            addrs = getattr(self.args, "address", None)
            if not addrs:
                raise ValueError(f"Model '{model_name}' is not an OpenAI model, but no vLLM addresses provided in config.env.response_agent.address")
            self.client = AsyncOpenAI(api_key="dummy", base_url=addrs)
    ...
    def prepare_history_message(self, retain_most_turns=1) -> str:
        ...
        """
        if len(self.trajectory_steps) <= retain_most_turns - 1:
            return "", ""
        ...
        num_compress = len(self.trajectory_steps) - retain_most_turns
        compress_steps = self.trajectory_steps[:num_compress] if num_compress > 0 else self.trajectory_steps
        ...
        history_to_append = ""
        for step in self.trajectory_steps[-retain_most_turns:]:
            thought = step.thought
            action = step.action
            observation = step.observation
    ...
    async def _start_memory_compression_async(
        self,
        history_to_compress: str,
    ):
        if history_to_compress.strip() == "":
            # Return zeroed metrics when there is nothing to summarize
            return "", 0, 0.0, "", 0, None
        summary_prompt = compressor_prompt_v5.format(
            conversation=history_to_compress
        )
        ...
        summary_exec_time = time.time() - start_time
        response_text = response.choices[0].message.content.strip()
        finish_reason = response.choices[0].finish_reason

        summary_tokens = self._count_summary_tokens(response_text)

        summary_prompt_tokens = self._count_tokens(
            [{"role": "user", "content": summary_prompt}]
        )

        return response_text, summary_tokens, summary_exec_time, summary_prompt, summary_prompt_tokens, finish_reason

    def _prepare_messages_for_query(self, summary: str, history_to_append: str, stepcount_message: str) -> List[Dict[str, str]]:
        if summary.strip() != "":
            formatted_prompt = (
    ...
    def run(
        ...
        while not done:
            ...
            # Step 1: Retrieve summaries from previous step's async task (if available)
            if pending_memory_task is not None:
                ready_summary, summary_tokens, summary_exec_time, summary_prompt, summary_prompt_tokens, finish_reason = loop.run_until_complete(pending_memory_task)
            else:
                ready_summary = ""
                summary_tokens = 0
                summary_exec_time = 0
            ...
            history_to_compress, history_to_append = self.prepare_history_message(
            ...
            pending_memory_task = loop.create_task(
                self._start_memory_compression_async(
            ...
            total_step_time = max(llm_exec_time, summary_exec_time) + env_exec_time if pending_memory_task is not None else llm_exec_time + env_exec_time
            ...
            trajectory_step = TrajectoryStep(
                ...
                query_prompt=messages[1]["content"] if ready_summary != "" else "",
                history_to_append=history_to_append,
                assistant_message=assistant_message if done and exit_reason == "token_limit" else "",
                # tokens
