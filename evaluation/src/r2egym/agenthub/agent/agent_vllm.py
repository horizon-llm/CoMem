# a version of agent that uses vLLM backend
import os
import re
import copy
import yaml
...
class Agent:
    ...
    def __init__(self, name: str, args: AgentArgs, logger=None):
        ...
        self.client = OpenAI(base_url=self.llm_base_url, api_key="")
    ...
    def run(
        ...
        while not done:
            ...
            try:
                response, llm_exec_time, llm_start_time, llm_end_time, vllm_metrics = self.model_query(messages, temperature)
        ...
        self.trajectory = Trajectory(
            ...
            total_time_traj=time.perf_counter() - traj_start_time,
            traj_start_time=traj_start_time,
            traj_finish_time=time.perf_counter(),
        )

        self.logger.info(f"Agent completed in {time.perf_counter() - traj_start_time} seconds.")
