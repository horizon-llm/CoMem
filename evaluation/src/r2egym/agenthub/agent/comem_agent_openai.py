...
class COMEMAgent:
    ...
    def __init__(self, name: str, args: AgentArgs, logger=None):
        ...
        self.mem_tokenizer = tiktoken.get_encoding("o200k_base")

        self.agent_client = AsyncOpenAI(base_url=self.llm_base_url, api_key="dummy")
        self.mem_client = AsyncOpenAI(api_key=os.environ.get("OPENAI_API_KEY", "dummy"))
    ...
    async def _start_memory_compression_async(
        ...
        call_params["max_completion_tokens"] = self.args.memory_model_max_gen_tokens
        call_params["timeout"] = self.llm_timeout
        ...
        if len(response_text) == 0:
            raise ValueError("Empty summary received from memory compression: " + str(response))
        ...
        while not done:
            ...
            if pending_memory_task is not None:
                try:
                    ...
                    self.logger.info(f"SUMMARY: {ready_summary}\n")
                except Exception as e:
