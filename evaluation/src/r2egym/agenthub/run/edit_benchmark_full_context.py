"""
Entry point for full-context agent latency benchmarking.

Supports both single benchmarks and concurrent batch execution
(via ProcessPoolExecutor) to measure latency under load.

Unlike the COMEM benchmark, this uses a single model with no
memory/summarization — the full conversation history is kept in context.

Usage:
    # Single benchmark
    python src/r2egym/agenthub/run/edit_benchmark_full_context.py run_benchmark \
        --input_token_target 4096 --output_token_target 2048 --max_steps 20

    # Concurrent batch benchmark
    python src/r2egym/agenthub/run/edit_benchmark_full_context.py run_benchmark_batch \
        --input_token_target 4096 --output_token_target 2048 \
        --num_instances 32 --max_workers 32

    # Grid over multiple (input, output) configs
...
from r2egym.agenthub.agent.agent_vllm import AgentArgs
from r2egym.agenthub.agent.fullcontext_benchmark_agent import BenchmarkFullContextAgent
...
def run_benchmark(
    ...
    """Run a single full-context benchmark instance.
    ...
    Args:
        ...
        max_total_time: Overall agent run time limit.
    ...
    """
    if exp_name is None:
        exp_name = f"fc_bench_in{input_token_target}_out{output_token_target}"
    ...
    bench_logger.info(
        f"[Instance {instance_id}] Starting full-context benchmark: "
    ...
    agent = BenchmarkFullContextAgent(
        name=f"BenchmarkFullContextAgent_{instance_id}",
        args=agent_args,
    ...
    # Run the agent (uses the real Agent.run() loop)
    try:
...
def run_benchmark_batch(
    ...
    """Run multiple full-context benchmark instances concurrently.
    ...
    if exp_name is None:
        exp_name = (
            f"fc_bench_in{input_token_target}_out{output_token_target}"
    ...
    logger.warning(
        f"Starting full-context batch benchmark: {num_instances} instances, "
        f"{max_workers} workers, input={input_token_target}, "
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        if latency_test:
            for future in concurrent.futures.as_completed(future_to_id):
                ...
                logger.warning(
                    f"  Progress: {completed + failed}/{num_instances} "
                    f"({completed} ok, {failed} failed) [{elapsed:.1f}s elapsed]"
                )
        else:
    ...
    if latency_test:
        ...
        with open(latency_file, "w") as f:
            f.write(f"Full-Context Benchmark Configuration:\n")
    ...
    # Print summary
    print(f"\n{'='*80}")
    print(f"FULL-CONTEXT BATCH BENCHMARK RESULTS")
    print(f"{'='*80}")
...
def run_benchmark_grid(
    ...
    """Run full-context batch benchmarks across a grid of configurations.
    ...
    if exp_name is None:
        exp_name = f"fc_bench_grid_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    summary_results = []
    total_configs = (
        len(input_targets) * len(output_targets)
        ...
        for out in output_targets:
            for steps in steps_targets:
                for workers in workers_targets:
                    config_idx += 1
                    config_exp = f"in{inp}_out{out}_s{steps}_w{workers}_n{num_instances}"
                    logger.warning(
                        f"[{config_idx}/{total_configs}] Running full-context batch benchmark: "
                        f"input={inp}, output={out}, steps={steps}, "
    ...
    print(f"\n{'='*135}")
    print("FULL-CONTEXT GRID BENCHMARK SUMMARY")
    print(f"{'='*135}")
    print(
        f"{'Input':>8} {'Output':>8} {'Steps':>7} {'Workers':>8} {'Inst':>6} {'TotSteps':>9} "
        f"{'AvgLLM(s)':>10} {'AvgStep(s)':>11} "
