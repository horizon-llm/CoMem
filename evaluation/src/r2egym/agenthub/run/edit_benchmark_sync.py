"""
Entry point for SYNCHRONOUS COMEM-KSO latency benchmarking.

Identical to edit_benchmark.py but uses SyncBenchmarkCOMEMAgent, which runs
memory compression and agent model queries sequentially (no async overlap).

This enables a direct comparison:
  - edit_benchmark.py      -> async (interleaved) execution
  - edit_benchmark_sync.py -> sync (sequential) execution

The latency difference between the two isolates the contribution of the
asynchronous systems design vs. the learned memory compression itself.

Usage:
    python src/r2egym/agenthub/run/edit_benchmark_sync.py run_benchmark_grid \
        --input_token_targets "2048,4096" --output_token_targets "1024,2048" \
        --num_instances 16 --max_workers 16
"""
...
from r2egym.agenthub.agent.comem_benchmark_agent_sync import SyncBenchmarkCOMEMAgent
...
def run_benchmark(
    ...
    """Run a single SYNCHRONOUS benchmark instance."""
    if exp_name is None:
        exp_name = f"bench_sync_in{input_token_target}_out{output_token_target}"
    ...
    bench_logger.info(
        f"[Instance {instance_id}] Starting SYNC benchmark: input={input_token_target}, "
    ...
    agent_args.use_optimized_schedule = False  # no optimized schedule in sync mode
    ...
    agent = SyncBenchmarkCOMEMAgent(
        name=f"SyncBenchmarkAgent_{instance_id}",
        args=agent_args,
...
def run_benchmark_batch(
    ...
    """Run multiple SYNCHRONOUS benchmark instances concurrently."""
    if max_workers is None:
        max_workers = num_instances

    if exp_name is None:
        exp_name = (
            f"bench_sync_in{input_token_target}_out{output_token_target}"
    ...
    logger.warning(
        f"Starting SYNC batch benchmark: {num_instances} instances, {max_workers} workers, "
    ...
    if latency_test:
        ...
        with open(latency_file, "w") as f:
            f.write(f"Benchmark Configuration (SYNCHRONOUS):\n")
    ...
    logger.warning(
        f"SYNC batch benchmark completed: {len(results) if latency_test else 'N/A'} instances "
        f"in {elapsed_time:.2f}s"
    )

    print(f"\n{'='*80}")
    print(f"SYNC BATCH BENCHMARK RESULTS")
    print(f"{'='*80}")
...
def run_benchmark_grid(
    ...
    """Run SYNCHRONOUS batch benchmarks across a grid of configurations."""
    ...
    if exp_name is None:
        exp_name = f"bench_sync_grid_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            ...
            for mem_out in mem_out_targets:
                for steps in steps_targets:
                    for workers in workers_targets:
                        ...
                        config_exp = f"sync_in{inp}_out{out}_{mem_label}_s{steps}_w{workers}_n{num_instances}"
                        logger.warning(
                            f"[{config_idx}/{total_configs}] Running SYNC batch benchmark: "
    ...
    print(f"\n{'='*155}")
    print("SYNC GRID BENCHMARK SUMMARY")
    print(f"{'='*155}")
    print(
